...
**Live updates:** `GET /api/polls/{publicId}/events` is an SSE stream. There's no Durable Object/pub-sub yet, so it re-projects and emits a full `PollView` snapshot on a 3s interval, only sending when the payload actually changed. The frontend (`src/components/poll-view/usePoll.ts`) only opens this stream once results become visible to the viewer (voted or expired), since the EventSource can't carry the management-key header — `canManage` is pinned from the last keyed load and re-applied to every SSE frame client-side.
